#!/usr/bin/perl
###############################################################################
# Script Name : citation_matcher.pl
...
# Version     : 1.1
#
# Usage       : perl citation_ids.pl <xml_file>
#               perl citation_ids.pl --help
#
...
sub build_patterns {
    my ($bibcit_id) = @_;
    log_debug("No bibcit_id provided") unless defined $bibcit_id;
...
sub process_bibcit_id {
    ...
    log_debug("No XML provided") unless $xml;
    log_debug("No bibcit_id provided") unless $bibcit_id;
...
sub find_matching_patterns_source_xml {
    ...
    # log_info("Patterns matched in source XML: ");
...
sub main {
    ...
    log_debug("XML file does not exist: $xml_file") unless -e $xml_file;
    log_debug("No bibcit IDs provided") unless $bibcit_ids =~ /\S/;
    ...
    foreach my $key (keys %{$global_patterns}) {
        if ($pattern_found && $pattern_found eq $global_patterns->{$key}) {
            if ($key eq 'pat1-with-bracket' || $key eq 'pat2-with-bracket') {
                $final_output = "(".join('; ', @most_common_patterns_list).")";
                ...
                $final_output = "($final_pattern_str)";
            }
            last;
        }
    }
    ...
    if ($final_output) {
        ...
        $final_output = "No patterns found for the provided bibcit IDs.";
    }

    my %output = ( result => $final_output );
