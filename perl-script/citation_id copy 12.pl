...
sub process_most_common_patterns {
    ...
    print("\nProcessing most common patterns...\n @patterns\n");
...
sub find_matching_patterns_source_xml {
    ...
    print("Regex pattern for source XML: @{$regex_pattern}\n");
    my $count = 0;
    my $pattern_hash = {};
    ...
    foreach my $regex (@$regex_pattern) {
        ...
        while ($xml =~ /$regex/g) {
            ...
            if ($i == 1) {
                my $temp_str = $&;
                print("Pattern with 'and' found: $&\n");
                $count_bibcit = () = $temp_str =~ /<bibcit\b/gi;
                print("Dollar I======> is equal to 1 Exiting as this is the most common pattern type. $count_bibcit bibcit tags found. and required $total_bibcit_ids bibcit ids.\n");
            }
            ...
            $match =~ s/^\s+|\s+$//g;
            $count++;
            print("\nMatched pattern in source XML: Count => $count, $match\n");
            $xml =~ s/\Q$match\E//;
        }
        if( $i == 3) {
...
sub main {
    ...
    foreach my $key (keys %{$global_patterns}) {
        print("Checking pattern===> $key\n");
        print("Global pattern=====> $global_patterns->{$key}\n");
