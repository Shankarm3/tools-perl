...
while ($content =~ /([A-Z][a-z]+(?:,\s+[A-Z][a-z.]+)*)\s*\(\s*(<bibcit[^>]*ref-type="bibr"\s+[^>]*>(\d{4})<\/bibcit>)\s*\)/g) {
    my $author = $1;
    my $year = $2;
...
while ($content =~ /(\([^<]+?et al\.\s*,\s*(<bibcit[^>]*ref-type="bibr"\s+[^>]*>(\d{4})<\/bibcit>)[^)]*\))/g) {
    my $full_match = $1;
    my $year = $2;
    $citations{$full_match} = $year;
}
...
while ($content =~ /(\(\s*([A-Z][a-z]+(?:,\s+[A-Z][a-z.]+)*\s+<bibcit[^>]*ref-type="bibr"\s+[^>]*>(\d{4})<\/bibcit>\s*)\))/g) {
    my $full_match = $1;
    my $year = $2;
    $citations{$full_match} = $year;
}
