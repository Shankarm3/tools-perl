...
sub process_bibcits_no_uno {
    ...
    eval {
        ...
        print("result before restore unicode: $combined_output{result}\n");
...
sub process_figcit_references {
   ...
   foreach my $figcit_id (@figcit_ids) {
       $figcit_id =~ s/^\s+|\s+$//g;
       next unless $figcit_id;
       
       if ($xml =~ /<fig[^>]*?apt_id="\Q$figcit_id\E"[^>]*>(.*?)<\/fig>/s) {
           push @found_figcits, $1;
       } else {
           push @missing_ids, $figcit_id;
       }
   }

   my $result = {
       status  => "success",
       message => @missing_ids ? "Warning: The following figcit IDs were not found: " . join(', ', @missing_ids) : "",
       result  => join("", @found_figcits),
       timestamp => strftime("%a %b %d %H:%M:%S %Y", localtime)
   };
   
   $result->{missing_ids} = \@missing_ids if @missing_ids;
...
sub generate_figcit_tags {
   ...
   foreach my $range (@$ranges) {
       ...
       # If any ID in the range is missing, skip this range
       my @missing = grep { !exists $fig_info->{$_} } @ids;
       if (@missing) {
           push @missing_ids, @missing;
           next;
       }
   ...
   return (\@figcit_tags, \@missing_ids);
}
