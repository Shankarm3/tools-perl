...
sub main {
    ...
    eval {
        ...
        if ($is_numbered_reference == 0) {
            $xml =~ s/<ref[^<>]*type="arabic"[^<>]*>.*?<\/ref>//isx;
            for my $bibcit_id (@ids) {
                my $found = 0;
                eval {
                    if(check_bibcit_id_in_ref($xml, $bibcit_id)) {
            ...
            if (@not_found_ids) {
                ...
                if (keys %$author_info) {
                    ...
                    foreach my $id (@not_found_ids) {
                        if(check_bibcit_id_in_ref($xml, $id)) {
            ...
            my @bibcit_ids = process_bibcit_ids($xml, $bibcit_ids_str);
