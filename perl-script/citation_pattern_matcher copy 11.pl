...
sub main {
    ...
    eval {
        ...
        if ($is_numbered_reference == 0) {
            ...
            $combined_output{message} = "Some references were not found" if @not_found_ids;
            if (@not_found_ids) {
                my $author_info = get_authors_from_references($xml_copy_2, \@not_found_ids);
                my @filtered_not_found;
                ...
                if (@filtered_not_found) {
                    $combined_output{missing_references} = {
                        'ids' => \@filtered_not_found,
                    };
                    $combined_output{message} = "Some references were not found";
                }
            }
        }
        ...
        if (@bibcit_tags) {
            delete $combined_output{missing_references};
            $combined_output{message} = "" if exists $combined_output{message};
            
            my $numbered_refs = join("", @bibcit_tags);
...
sub print_results {
    ...
    my $json_output = {
        message => "",
        result => $output,
        status => length($output) > 0 ? "success" : "error",
        timestamp => scalar localtime
    };
    my $json = JSON->new->pretty->canonical->encode($json_output);
    print $json;
}
