...
sub process_bibcit_id_uno {
    ...
    print("Patterns for $bibcit_id: ", Dumper(@patterns));
    my $xml_copy = $xml;
    ...
    print("most common patterns list ===>", @{$most_common_patterns_list});
...
sub main {
    ...
    eval {
        ...
        if ($is_numbered_reference == 0) {
            ...
            if(scalar @{$output{'missing_references'}{'ids'}} > 0) {
                my $bibcit_ids_string = join(",", @{$output{'missing_references'}{'ids'}});
                my @bibcit_ids = process_bibcit_ids_no($xml, $bibcit_ids_string);
