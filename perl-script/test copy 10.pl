...
sub prepare_output {
    ...
    foreach my $citation (@$citations) {
        ...
        if($actual_replacement > 0){
            # log_info(sprintf("actualReplacement: %d", $actual_replacement));
            push @citations_json, {
                id => $count++,
                citationId => $citation,
...
sub sort_citations {
    ...
    $citation_text =~ s/e\.g\.//gi;
    $citation_text =~ s/see,\s+//gi;
    $citation_text =~ s/and\s+//gi;
    $citation_text =~ s/&[^;]+;//gi;
    $citation_text =~ s/\s+/ /g;
    ...
    if (@citations > 1) {
        ...
        if($original_citation_text ne join('; ', @citations)){
            # log_info(sprintf("sorted citation_text: %s", join('; ', @citations)));
            # log_info(sprintf("original_citation_text: %s", $original_citation_text));
            ${$counter_ref}++;
        }
    }
