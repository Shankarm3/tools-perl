...
sub main {
    ...
    eval {
        ...
        my ($modified_xml, $normalized_occurrences) = find_normalized_occurrences($xml_content, $word_hash, $separator_info);
...
sub find_normalized_occurrences {
    ...
    while (my ($original, $normalized) = each %$word_hash) {
        ...
        if ($original =~ /\s/) {
            ...
            my $working_xml = $modified_xml;
            while ($working_xml =~ /\b($quoted_original)\b/gi) {
                my $found_word = $1;
                $normalized_occurrences{$found_word} = $normalized;
                push @final_list, { original => $found_word, normalized => $normalized };
            }
            $modified_xml =~ s/\b($quoted_original)\b/$1 $replacement/gi;
            next;
        }
        ...
        foreach my $sep (keys %$separator_info) {
            ...
            my $working_xml = $modified_xml;
            while ($working_xml =~ /($quoted_pattern)/gi) {
                my $found_word = $1;
                $normalized_occurrences{$found_word} = $normalized;
                push @final_list, { original => $found_word, normalized => $normalized };
            }
            
            $modified_xml =~ s/($quoted_pattern)/$1 $replacement/gi;
        }
    }
    
    # Store the normalized occurrences in the result
    return wantarray ? (\@final_list, \%normalized_occurrences) : $modified_xml;
}
