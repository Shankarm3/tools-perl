...
foreach my $range (@ranges) {
    if ($range =~ /^(\d+)-(\d+)$/) {
        ...
        if (exists $num_to_section{$range}) {
            ...
            push @output, qq{$prefix <seccit rid="$section->{rid}" title="seccit" href="#" contenteditable="false" id="seccit_$max_apt_id" apt_id="$max_apt_id">$range</seccit>};
        }
    }
}
...
sub find_ranges {
    ...
    # Process whole numbers for ranges
    if (@whole) {
